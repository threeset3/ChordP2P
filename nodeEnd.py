...
class Node:
	...
	def find_predecessor(self, req_node):
		...
		if(self.node_id == self.ft[0] and self.node_id==self.predecessor):
			...
			#new node falls between self and self.successor - case 2
			...
			#new node is the new biggest node - case 3
	...
	def update_finger_table(self, new_node, dest_node, idx):
		...
		if(self.sock.has_key(dest_node)):
			if(self.sock[dest_node].sendall(request)==0):
				pass
			else:
	...
	def update_table_handler(self, idx, finger):
		...
		if(idx == 6 and finger == 80):
			print '[Node %d]*****************idx=6 and finger=80*************\n '%self.node_id
		if(finger > self.node_id):
			accept = 0
			if(finger < self.ft[idx] or self.ft[idx] == 0):
				accept = 1
			...
			if(accept):
				...
				print '[Node %d] node %d change your idx %d value to %d \n'%(self.node_id,self.predecessor, idx, finger)
